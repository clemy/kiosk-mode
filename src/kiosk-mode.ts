...
class KioskMode implements KioskModeRunner {
	...
	protected async insertMoreInfoDialogStyles() {
		...
		if (
			...
			options[OPTION.HIDE_DIALOG_HEADER_ADD_ENTITY_TO] ||
			...
			const styles = [
				...
				(options[OPTION.HIDE_DIALOG_HEADER_ACTION_ITEMS] || options[OPTION.HIDE_DIALOG_HEADER_ADD_ENTITY_TO])
					&& STYLES.DIALOG_HEADER_ADD_ENTITY_TO,
			...
			if (queryString(SPECIAL_QUERY_PARAMS.CACHE)) {
				...
				if (options[OPTION.HIDE_DIALOG_HEADER_ADD_ENTITY_TO])         setCache(TRUE, OPTION.HIDE_DIALOG_HEADER_ADD_ENTITY_TO);
